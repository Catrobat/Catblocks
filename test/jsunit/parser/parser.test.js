...
describe('Catroid to Catblocks parser tests', () => {
  ...
  describe('Formula function tests', () => {
    ...
    test('formular with 3 arguments', async () => {
      const blockName = 'SetXBrick';
      const categoryName = 'X_POSITION';
      const first = 1;
      const second = 2;
    ...
    test('multiple formular', async () => {
      ...
      const output = [
        ...
        "join('hello',' world','!')",
        'flatten(*List (1)*)',
        "item's index( 1 , *List (1)*)",
        ' look direction ',
        ' number of looks ',
        ' motion direction ',
        ' user language ',
        ' number of current touches ',
        'index of current touch( 1 )',
        ' timer '
      ];
      const first = [
        'FALSE',
      ...
      const second = ['2', 'LENGTH', ' world', 'FLATTEN', 'List (1)', 'INDEX_CURRENT_TOUCH'];
      const third = ['3', '!', 'INDEX_OF_ITEM'];
      const xmlString = `<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
      <program>
        <header>
          <programName>formulars</programName>
          <catrobatLanguageVersion>0.99997</catrobatLanguageVersion>
        </header>
        <settings/>
        <scenes>
