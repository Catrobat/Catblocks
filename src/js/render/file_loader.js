...
export class FileLoader {
  ...
  fixCorsForGithubUrl() {
    if(this.url.startsWith("https://github.com")) {
      this.downloadUrl = this.url.replace("https://github.com", "https://raw.githubusercontent.com").replace("/raw/", "/");
    }
  }
      ...
      fetch(this.downloadUrl)
        .then(resp => {
          if(resp.status == 200 && resp.ok) {
            ...
            zipPromise.then((result) => {
              if(result !== null) {
                try {
                  this.renderProgram(this.share, this.container, result.codeXML, this.url, containerCounter, result.fileMap);
                  updateView('onDone');
